  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        type === 'error' ? "bg-red-500/20 border-red-500/50 text-red-200" : "bg-green-500/20 border-green-500/50 text-green-200"
      )}
    >
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Reward Shop</h1>
          <p className="text-slate-400 mt-1">Spend your hard-earned coins on exclusive cosmetics.</p>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/30 rounded-xl">
          <RiCopperCoinLine className="w-6 h-6 text-amber-400" />
          <motion.span key={coins} initial={{ scale: 1.5 }} animate={{ scale: 1 }} className="text-xl font-bold text-amber-400">
            {coins}
          </motion.span>
        </div>
      </div>
      ...
      <div className="space-y-10">
        {(['frame', 'badge', 'theme'] as const).map(category => {
          ...
          return (
            <div key={category}>
              <h2 className="text-xl font-semibold text-white capitalize mb-4">{category}s</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {items.map(item => {
                  ...
                  return (
                    <Card key={item.id} className={cn("relative overflow-hidden transition-all", isEquipped && "ring-2 ring-indigo-500 bg-indigo-500/5")}>
                      <div className="p-4 flex flex-col items-center text-center">
                        <div className={cn("w-16 h-16 rounded-2xl flex items-center justify-center mb-3", isPurchased ? "bg-indigo-500/20 text-indigo-400" : "bg-slate-800 text-slate-500")}>
